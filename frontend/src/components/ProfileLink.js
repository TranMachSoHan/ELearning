...
const ProfileLink = ({imgSrc, name, role}) => {
    ...
    return ( <div className="relative flex items-center gap-4 cursor-pointer " onMouseEnter={() => {setOpen(true)}} onMouseLeave={() => {setOpen(false)}}>
        ...
        <ul className={"absolute right-0 -bottom-0 transform translate-y-full z-10 p-4 space-y-2 bg-white drop-shadow-lg " + (open ? 'block' : 'hidden')}>
